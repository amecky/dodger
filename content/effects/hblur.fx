...
OutputVS HBlurVS(float3 posL : POSITION0, float2 tex : TEXCOORD0, float4 color : COLOR0) {
	OutputVS outVS = (OutputVS)0;
	outVS.posH = mul(float4(posL, 1.0f), gWVP);
	outVS.tex0 = tex;
	outVS.color0 = color;
    outVS.texCoord1 = tex + float2(texelSize * -3.0, 0.0);
    outVS.texCoord2 = tex + float2(texelSize * -2.0, 0.0);
    outVS.texCoord3 = tex + float2(texelSize * -1.0, 0.0);
    outVS.texCoord4 = tex + float2(texelSize *  0.0, 0.0);
    outVS.texCoord5 = tex + float2(texelSize *  1.0, 0.0);
    outVS.texCoord6 = tex + float2(texelSize *  2.0, 0.0);
    outVS.texCoord7 = tex + float2(texelSize *  3.0, 0.0);
	return outVS;
}

float4 HBlurPS(OutputVS input) : COLOR{
...
technique HBlurTech {
	pass P0 {
		vertexShader = compile vs_3_0 HBlurVS();
